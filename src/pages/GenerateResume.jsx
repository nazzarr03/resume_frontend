...
import { FaBrain, FaTrash, FaPaperPlane, FaPlusCircle, FaTrash as FaTrashIcon } from "react-icons/fa";
...
const GenerateResume = () => {
  ...
  // Form submit olunca state ve görünürlükleri ayarla
  const onSubmit = (formData) => {
    ...
    // Form gönderildiğinde PDF oluştur ve indir
    setTimeout(() => {
      generatePDF();
    }, 100); // DOM güncellemesinin tam uygulanması için küçük gecikme ekledim
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // API'den gelen veriyi varsa defaultData ile merge et (opsiyonel)
      const mergedData = { ...defaultData, ...responseData };

      reset(mergedData); // Formu API cevabıyla doldur
      setData(mergedData);
  ...
  const renderInput = (name, label, type = "text") => (
    <div className="form-control w-full mb-4" key={name}>
      ...
      {fields.fields.map((field, index) => (
        <div key={field.id} className="p-4 rounded-lg mb-4 bg-base-100">
          ...
          <button
            ...
            className="btn btn-error btn-sm mt-2 flex items-center gap-2"
          >
            <FaTrashIcon className="w-5 h-5 text-base-content" /> Remove {label}
          </button>
        </div>
      ))}
      <button
  ...
  return (
    <div className="mt-5 p-10 flex flex-col gap-3 items-center justify-center font-sans">
      ...
      {showResumeUI && <Resume data={data} ref={resumeRef} />}
    </div>
  );
};
